...
class PhoenixClient:
    ...
    def _make_request(self, method: str, endpoint: str, **kwargs) -> Dict:
        ...
        try:
            ...
            if (
                "application/x-pandas-arrow" in content_type
                or "application/vnd.apache.arrow.stream" in content_type
            ):
                # Parse Arrow format
    ...
    def get_spans(
        ...
        if start_time or end_time:
            try:
                ...
                def _iso_utc(dt: datetime) -> str:
                    ...
                    return (
                        dt.astimezone(timedelta(0)).isoformat().replace("+00:00", "Z")
                    )
        ...
        def _post(query_text: str) -> Dict:
            ...
            response = requests.post(
                url, headers=headers, json={"query": query_text}, timeout=30
            )
            response.raise_for_status()
            return response.json()
        ...
        try:
            ...
            if "errors" in data and filter_str:
                logger.warning(
                    "GraphQL rejected time filter; retrying without server-side time filtering."
                )
    ...
    def search_spans(
        self, query: str, project_id: Optional[str] = None, limit: int = 100
    ) -> List[Dict]:
        """
