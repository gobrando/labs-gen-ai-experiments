...
class TraceAnalyzer:
    ...
    def get_macro_statistics(self) -> Dict:
        ...
        if (
            "trace_id" in self.df.columns
            and "name" in self.df.columns
            and "parent_id" in self.df.columns
        ):
            ...
            # Find root spans (spans with no parent_id or parent_id not in the trace)
            for trace_id, trace_spans in traces.items():
                span_ids = {str(s.get("span_id", "")) for s in trace_spans}
                root_spans = []
                for s in trace_spans:
                    parent_id = s.get("parent_id", "")
                    if pd.isna(parent_id) or str(parent_id) not in span_ids:
                        root_spans.append(s)
        ...
        stats = {
            ...
            "unique_traces": self.df["trace_id"].nunique()
            ...
            "avg_latency_s": self.df["latency_s"].mean()
            if "latency_s" in self.df.columns
            else 0,
            "median_latency_s": self.df["latency_s"].median()
            if "latency_s" in self.df.columns
            else 0,
            "p95_latency_s": self.df["latency_s"].quantile(0.95)
            if "latency_s" in self.df.columns
            else 0,
            "p99_latency_s": self.df["latency_s"].quantile(0.99)
            if "latency_s" in self.df.columns
            else 0,
            "total_tokens": self.df["token_count_total"].sum()
            if "token_count_total" in self.df.columns
            else 0,
            "avg_tokens_per_request": self.df["token_count_total"].mean()
            if "token_count_total" in self.df.columns
            else 0,
            "error_rate": (self.df["status_code"] == "ERROR").sum() / len(self.df) * 100
            if "status_code" in self.df.columns
            else 0,
            "success_rate": (self.df["status_code"] == "OK").sum() / len(self.df) * 100
            if "status_code" in self.df.columns
            else 0,
        }
    ...
    def get_latency_distribution(self) -> Dict:
        ...
        if self.df.empty or "latency_s" not in self.df.columns:
            return {}

        latency_data = self.df["latency_s"].dropna()
        ...
        for trace_id, trace_spans in traces.items():
            ...
            if isinstance(attrs, dict):
                ...
                if isinstance(user_info, dict):
                    user_id = str(
                        user_info.get("id", "") or user_info.get("email", "")
                    ).strip()
                ...
                if isinstance(metadata, dict):
                    user_email = str(
                        metadata.get("user_id", "") or metadata.get("user_email", "")
                    ).strip()
                    if not user_id:
                        user_id = user_email
            ...
            if trace_type == "email_results" and (
                not user_email or user_email.startswith("unknown_")
            ):
                                            ...
                                            if isinstance(data, dict):
                                                ...
                                                if not email_found:
                                                    ...
                                                    if isinstance(logger_data, dict):
                                                        messages = logger_data.get(
                                                            "messages_list", []
                                                        )
                                                        if messages and isinstance(
                                                            messages[0], dict
                                                        ):
                                                            email_found = messages[
                                                                0
                                                            ].get("user_email", "")
    ...
    def analyze_output_quality(self) -> Dict:
        ...
        for trace_id in self.traces_df["trace_id"].unique():
            ...
            trace_row = (
                self.traces_df[self.traces_df["trace_id"] == trace_id].iloc[0]
                if not self.traces_df[self.traces_df["trace_id"] == trace_id].empty
                else None
            )
                ...
                if isinstance(output_attr, str) and output_attr:
                    ...
                    try:
                        output_data = (
                            ...
                            if output_attr.startswith("{")
                            or output_attr.startswith("[")
                            else {}
                        )
        ...
        return {
            ...
            "insights": insights,
            "all_analyses": sorted(output_analyses, key=lambda x: x["quality_score"])[
                :50
            ],  # Worst 50 for review
