...
class TraceAnalyzer:
    ...
    def _apply_excluded_user_filter(
        ...
        if self.traces_df is None or self.traces_df.empty:
            return
        if (
            "user_email" not in self.traces_df.columns
            or "trace_id" not in self.traces_df.columns
        ):
            return
    ...
    def _create_dataframe(self, spans: List[Dict]) -> pd.DataFrame:
        ...
        for span in spans:
            ...
            processed["input"] = attributes.get(
                "input", attributes.get("llm.input_messages", "")
            )
            processed["output"] = attributes.get(
                "output", attributes.get("llm.output_messages", "")
            )
            processed["model"] = attributes.get(
                "llm.model_name", attributes.get("model", "")
            )
            processed["token_count_prompt"] = attributes.get(
                "llm.token_count.prompt", 0
            )
            processed["token_count_completion"] = attributes.get(
    ...
    def get_macro_statistics(self) -> Dict:
        ...
        durations = pd.to_numeric(
            latency_source.get("trace_duration_s"), errors="coerce"
        )
        ...
        if "trace_type" in self.traces_df.columns:
            counts = (
                self.traces_df["trace_type"].fillna("other").value_counts().to_dict()
            )
            for k in trace_counts.keys():
                trace_counts[k] = int(counts.get(k, 0))
            known = set(trace_counts.keys())
            trace_counts["other"] += int(
                sum(v for t, v in counts.items() if t not in known)
            )
        ...
        stats = {
            ...
            "unique_traces": self.traces_df["trace_id"].nunique()
            if "trace_id" in self.traces_df.columns
            else 0,
            "trace_counts": trace_counts,
            "date_range": {
                "start": self.traces_df["trace_start"].min()
                if "trace_start" in self.traces_df.columns
                else None,
                "end": self.traces_df["trace_start"].max()
                if "trace_start" in self.traces_df.columns
                else None,
            },
            ...
            "total_tokens": int(self.traces_df["total_tokens"].sum())
            if "total_tokens" in self.traces_df.columns
            else 0,
            "avg_tokens_per_request": float(self.traces_df["total_tokens"].mean())
            if "total_tokens" in self.traces_df.columns
            else 0,
            "error_rate": (self.traces_df["status"] == "ERROR").sum()
            / len(self.traces_df)
            * 100
            if "status" in self.traces_df.columns
            else 0,
            "success_rate": (self.traces_df["status"] == "OK").sum()
            / len(self.traces_df)
            * 100
            if "status" in self.traces_df.columns
            else 0,
    ...
    def get_time_series_data(self, freq: str = "H") -> pd.DataFrame:
        ...
        time_series = (
            df_time.groupby(pd.Grouper(freq=freq))
            .agg(
                {
                    "span_id": "count",
    ...
    def get_latency_distribution(self) -> Dict:
        ...
        latency_data = pd.to_numeric(
            source["trace_duration_s"], errors="coerce"
        ).dropna()
    ...
    def get_error_analysis(self) -> Dict:
        ...
        return {
            ...
            "errors_by_type": error_df["name"].value_counts().to_dict()
            if "name" in error_df.columns
            else {},
            "errors_over_time": error_df.groupby(pd.Grouper(key="start_time", freq="D"))
            .size()
            .to_dict()
            if "start_time" in error_df.columns
            else {},
        }
    ...
    def get_quality_insights(self) -> pd.DataFrame:
        ...
        if "latency_s" in quality_df.columns:
            # Flag slow responses
            quality_df["is_slow"] = quality_df["latency_s"] > quality_df[
                "latency_s"
            ].quantile(0.90)
    ...
    def search_interactions(self, query: str) -> pd.DataFrame:
        ...
        mask = self.df["input"].astype(str).str.contains(
    ...
    def get_token_usage_analysis(self) -> Dict:
        ...
        return {
            ...
            "tokens_by_model": self.df.groupby("model")["token_count_total"]
            .sum()
            .to_dict()
            if "model" in self.df.columns
            else {},
    ...
    def get_model_performance_comparison(self) -> pd.DataFrame:
        ...
        if len(comparison) > 0:
            comparison["error_rate"] = (
                comparison["error_count"] / comparison["total_requests"] * 100
            ).round(2)
    ...
    def _extract_user_email_from_spans(
        ...
        trace_id = (
            trace_spans[0].get("trace_id", "unknown") if trace_spans else "unknown"
        )
    ...
    def _extract_emails_from_json_string(self, text: str) -> List[str]:
        ...
        for email in found:
            ...
            if not any(
                x in email.lower() for x in ["example.com", "test.com", "placeholder"]
            ):
                emails.append(email)
    ...
    def _create_traces_dataframe(self, spans: List[Dict]) -> pd.DataFrame:
        ...
        for trace_id, trace_spans in traces.items():
            ...
            category = (
                context.get("categories", [])[0] if context.get("categories") else ""
            )
            ...
            start_times = [
                pd.to_datetime(s.get("start_time", ""))
                for s in trace_spans
                if s.get("start_time")
            ]
            end_times = [
                pd.to_datetime(s.get("end_time", ""))
                for s in trace_spans
                if s.get("end_time")
            ]
            ...
            trace_duration = (
                (trace_end - trace_start).total_seconds()
            ...
            # Count spans by type
            llm_spans = [
                s for s in trace_spans if "llm" in str(s.get("span_kind", "")).lower()
            ]
            ...
            def _extract_span_total_tokens(span_obj: Dict) -> int:
                ...
                flat = (
                    attrs_obj.get("llm.token_count.total")
                    or attrs_obj.get("llm.token_count_total")
                    or attrs_obj.get("token_count_total")
                )
            ...
            if total_tokens == 0:
                ...
                for span in trace_spans:
                    ...
                    estimated += _estimate_tokens_from_value(
                        span_attrs.get("llm.input_messages")
                    )
                    estimated += _estimate_tokens_from_value(
                        span_attrs.get("llm.output_messages")
                    )
            ...
            trace_records.append(
                {
                    ...
                    "all_zip_codes": ",".join(zip_codes[:5])
                    if zip_codes
                    else "",  # Store up to 5 zips
    ...
    def _extract_context_from_payload(self, payload: Dict) -> Dict[str, List[str]]:
        ...
        """
        context = {"queries": [], "categories": [], "locations": []}
    ...
    def _extract_location_names(self, text: str) -> List[str]:
        ...
        known_locations = [
            ...
            "west lake hills",
            "lancaster",
            "virginia beach",  # Out of region mentions
        ]
    ...
    def _extract_trace_context(self, trace_spans: List[Dict]) -> Dict[str, List[str]]:
        ...
        """
        context = {"query": "", "categories": [], "location": "", "zip_codes": []}
                    ...
                    if isinstance(data, dict):
                        ...
                        if isinstance(prompt_builder, dict):
                            ...
                            if resources_text:
                                ...
                                inferred_cat = self._infer_category_from_resources(
                                    resources_text
                                )
                                if (
                                    inferred_cat
                                    and inferred_cat not in context["categories"]
                                ):
                ...
                if not context["location"]:
                    for key in [
                        ...
                        if metadata.get(key):
                            ...
                            self._append_zip_from_text(
                                context["location"], context["zip_codes"]
                            )
                            break
                meta_category = metadata.get("resource_category") or metadata.get(
                    "category"
                )
    ...
    def get_user_analytics(self) -> pd.DataFrame:
        ...
        user_stats = (
            self.traces_df.groupby("user_email")
            .agg(
                {
                    "trace_id": "count",
                    "trace_type": lambda x: {
        ...
        user_stats["referrals_count"] = user_stats["trace_type_breakdown"].apply(
            lambda x: x.get("referrals", 0) if isinstance(x, dict) else 0
        )
        user_stats["action_plans_count"] = user_stats["trace_type_breakdown"].apply(
            lambda x: x.get("action_plans", 0) if isinstance(x, dict) else 0
        )
        user_stats["email_results_count"] = user_stats["trace_type_breakdown"].apply(
            lambda x: x.get("email_results", 0) if isinstance(x, dict) else 0
        )
        user_stats["other_count"] = user_stats["trace_type_breakdown"].apply(
            lambda x: x.get("other", 0) if isinstance(x, dict) else 0
        )
    ...
    def get_trace_time_series(self, freq: str = "H") -> pd.DataFrame:
        ...
        time_series = (
            df_time.groupby(pd.Grouper(freq=freq))
            .agg(
                {"trace_id": "count", "trace_duration_s": "mean", "total_tokens": "sum"}
            )
            .reset_index()
        )
        ...
        referrals_ts = (
            df_time[df_time["trace_type"] == "referrals"]
            .groupby(pd.Grouper(freq=freq))
            .size()
        )
        action_plans_ts = (
            df_time[df_time["trace_type"] == "action_plans"]
            .groupby(pd.Grouper(freq=freq))
            .size()
        )
        email_results_ts = (
            df_time[df_time["trace_type"] == "email_results"]
            .groupby(pd.Grouper(freq=freq))
            .size()
        )
        ...
        time_series["referrals_count"] = referrals_ts.reindex(
            time_series.index, fill_value=0
        )
        time_series["action_plans_count"] = action_plans_ts.reindex(
            time_series.index, fill_value=0
        )
        time_series["email_results_count"] = email_results_ts.reindex(
            time_series.index, fill_value=0
        )
    ...
    def get_usage_breakdown_by_level(self) -> Dict:
        ...
        for email, count in user_counts.items():
            ...
            user_data = {"email": email, "name": user_name, "count": count}
        ...
        return {"high": high_users, "medium": medium_users, "low": low_users}
    ...
    def get_resource_category_breakdown(self) -> Dict:
        ...
        category_breakdown = dict(
            sorted(
                category_breakdown.items(), key=lambda x: x[1]["count"], reverse=True
            )
        )
    ...
    def extract_zip_codes(self) -> Dict:
        ...
        if not zip_codes:
            return {"all_zips": [], "texas": [], "out_of_state": [], "total_unique": 0}

        # Central Texas ZIP code to city mapping
        zip_to_city = {
            # Austin area
            "78701": "Austin (Downtown)",
            "78702": "Austin (East)",
            "78703": "Austin (West)",
            "78704": "Austin (South)",
            "78705": "Austin (Central)",
            "78712": "Austin (UT)",
            "78721": "Austin (East)",
            "78722": "Austin (Central)",
            "78723": "Austin (Northeast)",
            "78724": "Austin (East)",
            "78725": "Austin (Southeast)",
            "78726": "Austin (West)",
            "78727": "Austin (North)",
            "78728": "Austin (North)",
            "78729": "Austin (North)",
            "78730": "Austin (West)",
            "78731": "Austin (Northwest)",
            "78732": "Austin (West)",
            "78733": "Austin (West)",
            "78734": "Austin (West)",
            "78735": "Austin (Southwest)",
            "78736": "Austin (Southwest)",
            "78737": "Austin (South)",
            "78738": "Austin (West)",
            "78739": "Austin (South)",
            "78741": "Austin (South)",
            "78742": "Austin (Southeast)",
            "78744": "Austin (South)",
            "78745": "Austin (South)",
            "78746": "Austin (West)",
            "78747": "Austin (South)",
            "78748": "Austin (South)",
            "78749": "Austin (Southwest)",
            "78750": "Austin (Northwest)",
            "78751": "Austin (North)",
            "78752": "Austin (North)",
            "78753": "Austin (Northeast)",
            "78754": "Austin (North)",
            "78756": "Austin (Central)",
            "78757": "Austin (North)",
            "78758": "Austin (North)",
            "78759": "Austin (North)",
            # Surrounding cities
            ...
            "78642": "Leander",
            "78650": "Marble Falls",
            "78656": "Dripping Springs",
            # Out of region
            "77554": "Galveston (Houston area)",
            "75835": "Palestine, TX",
            "75765": "Nacogdoches, TX",
            "18102": "Allentown, PA",
        ...
        return {
            "all_zips": [
                {"zip": z, "city": zip_to_city.get(z, "Unknown"), "count": c}
                for z, c in zip_counts.most_common()
            ],
            "texas": sorted(texas_zips, key=lambda x: x["count"], reverse=True),
            "out_of_state": sorted(
                out_of_state, key=lambda x: x["count"], reverse=True
            ),
            "total_unique": len(zip_counts),
        }

    def get_cohort_analysis(
        self, cohort_emails: List[str], cohort_name: str = "Cohort"
    ) -> Dict:
        """
        ...
        for cohort_email in cohort_emails:
                ...
                for trace_email_lower, trace_email_orig in full_email_map.items():
                    if "@" in trace_email_lower:
                        ...
                        if trace_username == cohort_username:
                            ...
                            if (
                                cohort_domain in trace_domain
                                or trace_domain in cohort_domain
                                or cohort_domain.replace(".org", "") in trace_domain
                                or trace_domain.replace(".org", "") in cohort_domain
                            ):
            ...
            if matched_trace_email:
                ...
                user_traces = self.traces_df[
                    self.traces_df["user_email"].str.lower().str.strip()
                    == matched_trace_email.lower().strip()
                ]
                cohort_users.append(
                    {
                        "email": cohort_email,
                        "trace_email": matched_trace_email,
                        "name": cohort_email.split("@")[0].replace(".", " ").title()
                        if "@" in cohort_email
                        else cohort_email,
        ...
        return {
            ...
            "adoption_rate": (len(cohort_users) / len(cohort_emails) * 100)
            if cohort_emails
            else 0,
        }

    def get_comprehensive_report(
        self, start_date=None, end_date=None, cohort_emails=None
    ) -> Dict:
        """
        ...
        if start_date:
            filtered_df = filtered_df[
                filtered_df["trace_start"] >= pd.to_datetime(start_date)
            ]
        if end_date:
            filtered_df = filtered_df[
                filtered_df["trace_start"] <= pd.to_datetime(end_date)
            ]
        ...
        report = {
            "period": {
                "start": filtered_df["trace_start"].min()
                if len(filtered_df) > 0
                else None,
                "end": filtered_df["trace_start"].max()
                if len(filtered_df) > 0
                else None,
            },
            "totals": {
                ...
                "action_plans": len(
                    filtered_df[filtered_df["trace_type"] == "action_plans"]
                ),
            },
    ...
    def analyze_query_patterns(self) -> Dict:
        ...
        referral_users = set(
            queries_df[queries_df["trace_type"] == "referrals"]["user_email"]
        )
        action_plan_users = set(
            self.traces_df[self.traces_df["trace_type"] == "action_plans"]["user_email"]
        )
        ...
        return {
            ...
            "top_terms": dict(
                sorted(query_terms.items(), key=lambda x: x[1], reverse=True)[:20]
            ),
            "refinements": refinements[:10],  # Top 10 refinement examples
            "refinement_rate": len(refinements) / len(queries_df) * 100
            if len(queries_df) > 0
            else 0,
            "queries_without_action": len(no_action_queries),
            "no_action_rate": len(no_action_queries) / len(queries_df) * 100
            if len(queries_df) > 0
            else 0,
            "no_action_examples": no_action_queries[["user_email", "query", "category"]]
            .head(5)
            .to_dict("records"),
        }
    ...
    def analyze_resource_effectiveness(self) -> Dict:
        ...
        conversion_rate = (
            len(users_both) / len(users_searched) * 100
            if len(users_searched) > 0
            else 0
        )
        ...
        for category in self.traces_df["category"].unique():
            ...
            cat_conversion = (
                (cat_actions / cat_referrals * 100) if cat_referrals > 0 else 0
            )
        ...
        return {
            ...
            "top_categories_by_action": sorted(
                resources_by_category.items(),
    ...
    def analyze_user_journeys(self) -> Dict:
        ...
        for user, user_traces in user_groups:
            ...
            for idx, row in user_traces.iterrows():
                if session_start is None:
                    ...
                    time_diff = (
                        row["trace_start"] - session_traces[-1]["trace_start"]
                    ).total_seconds() / 60
                        ...
                        sessions.append(
                            {
                                ...
                                "duration_minutes": (
                                    session_traces[-1]["trace_start"] - session_start
                                ).total_seconds()
                                / 60,
        ...
        users_dropped = set(
            self.traces_df[self.traces_df["trace_type"] == "referrals"]["user_email"]
        ) - set(
            self.traces_df[self.traces_df["trace_type"] == "action_plans"]["user_email"]
        )

        dropped_traces = self.traces_df[
            self.traces_df["user_email"].isin(users_dropped)
        ]
        ...
        return {
            ...
            "avg_traces_per_session": np.mean([s["trace_count"] for s in sessions])
            if sessions
            else 0,
            "multi_category_sessions": len(multi_category),
            "multi_category_rate": len(multi_category) / len(sessions) * 100
            if sessions
            else 0,
            "avg_time_to_action": np.mean([t["time_minutes"] for t in time_to_action])
            if time_to_action
            else 0,
            "median_time_to_action": np.median(
                [t["time_minutes"] for t in time_to_action]
            )
            if time_to_action
            else 0,
            "users_with_action": len(time_to_action),
            "users_dropped": len(users_dropped),
            "drop_off_rate": len(users_dropped) / len(user_groups) * 100
            if len(user_groups) > 0
            else 0,
            "session_examples": sessions[:5],  # First 5 sessions as examples
            "time_to_action_examples": sorted(
                time_to_action, key=lambda x: x["time_minutes"]
            )[:5],
            "dropped_user_categories": dropped_traces["category"]
            .value_counts()
            .to_dict(),
        }
    ...
    def analyze_performance_quality(self) -> Dict:
        ...
        # Error rate analysis
        error_traces = (
            self.traces_df[self.traces_df["status"] == "ERROR"]
            if "status" in self.traces_df.columns
            else pd.DataFrame()
        )
        ...
        for category in self.traces_df["category"].unique():
            if category:
                ...
                cat_errors = (
                    len(error_traces[error_traces["category"] == category])
                ...
                error_rate_by_category[category] = {
                    "total": cat_total,
                    "errors": cat_errors,
                    "error_rate": (cat_errors / cat_total * 100)
                    if cat_total > 0
                    else 0,
                }
        ...
        token_usage = {
            "total_tokens": self.traces_df["total_tokens"].sum()
            if "total_tokens" in self.traces_df.columns
            else 0,
            "avg_tokens_per_trace": self.traces_df["total_tokens"].mean()
            if "total_tokens" in self.traces_df.columns
            else 0,
        ...
        if "total_tokens" in self.traces_df.columns:
            for category in self.traces_df["category"].unique():
                if category:
                    cat_tokens = self.traces_df[self.traces_df["category"] == category][
                        "total_tokens"
                    ]
        ...
        slow_traces = (
            self.traces_df[self.traces_df["trace_duration_s"] > slow_threshold]
            if "trace_duration_s" in self.traces_df.columns
            else pd.DataFrame()
        )
        ...
        return {
            ...
            "overall_error_rate": len(error_traces) / len(self.traces_df) * 100
            if len(self.traces_df) > 0
            else 0,
    ...
    def analyze_geographic_gaps(self) -> Dict:
        ...
        zip_to_city = {
            ...
            "78722": "Austin (Central)",
            "78723": "Austin (Northeast)",
            "78724": "Austin (Northeast)",
            "78725": "Austin (Southeast)",
            "78726": "Austin (Northwest)",
            "78727": "Austin (North)",
            "78728": "Austin (North)",
            "78729": "Austin (Northwest)",
            "78730": "Austin (West)",
            "78731": "Austin (Northwest)",
            "78732": "Austin (West)",
            "78733": "Austin (West)",
        ...
        for z in all_zips:
            if z in central_texas_zips:
                category_by_region["central_texas"][
                    zip_to_categories.get(z, ["Other"])[0]
                    if z in zip_to_categories
                    else "Other"
                ] += 1
            else:
                category_by_region["out_of_region"][
                    zip_to_categories.get(z, ["Other"])[0]
                    if z in zip_to_categories
                    else "Other"
                ] += 1
        ...
        return {
            ...
            "in_region_details": dict(
                sorted(in_region.items(), key=lambda x: x[1]["count"], reverse=True)
            ),
            "out_of_region_details": dict(
                sorted(out_of_region.items(), key=lambda x: x[1]["count"], reverse=True)
            ),
            ...
            "coverage_summary": {
                "austin_metro": sum(
                    1 for z in zip_counts.keys() if z.startswith("787")
                ),
                "round_rock": sum(
                    1 for z in zip_counts.keys() if z in ["78664", "78665", "78681"]
                ),
                "georgetown": sum(
                    1
                    for z in zip_counts.keys()
                    if z in ["78626", "78627", "78628", "78633"]
                ),
                "pflugerville": sum(1 for z in zip_counts.keys() if z == "78660"),
                "cedar_park_leander": sum(
                    1 for z in zip_counts.keys() if z in ["78613", "78641"]
                ),
                "south_austin": sum(
                    1 for z in zip_counts.keys() if z in ["78610", "78640", "78666"]
                ),
            },
        }

    def analyze_comparative_periods(
        self,
        ...
        def period_metrics(df):
            ...
            return {
                ...
                "avg_duration_s": df["trace_duration_s"].mean()
                if "trace_duration_s" in df.columns
                else 0,
        ...
        changes = {
            "traces_change": calc_change(
                p1_metrics["total_traces"], p2_metrics["total_traces"]
            ),
            "users_change": calc_change(
                p1_metrics["unique_users"], p2_metrics["unique_users"]
            ),
            "referrals_change": calc_change(
                p1_metrics["referrals"], p2_metrics["referrals"]
            ),
            "action_plans_change": calc_change(
                p1_metrics["action_plans"], p2_metrics["action_plans"]
            ),
            "duration_change": calc_change(
                p1_metrics["avg_duration_s"], p2_metrics["avg_duration_s"]
            ),
        }
        ...
        def daily_breakdown(df):
            ...
            daily = (
                ...
                .agg({"trace_id": "count", "user_email": "nunique"})
                .reset_index()
            )
        ...
        return {
            "period1": {
                ...
                if hasattr(period1_start, "isoformat")
                else str(period1_start),
                "end": period1_end.isoformat()
                if hasattr(period1_end, "isoformat")
                else str(period1_end),
                "metrics": p1_metrics,
            ...
            "period2": {
                ...
                if hasattr(period2_start, "isoformat")
                else str(period2_start),
                "end": period2_end.isoformat()
                if hasattr(period2_end, "isoformat")
                else str(period2_end),
                "metrics": p2_metrics,
            ...
            if changes["traces_change"] > 10
            else ("declining" if changes["traces_change"] < -10 else "stable"),
        }
    ...
    def analyze_realtime_alerts(self) -> Dict:
        ...
        if "total_tokens" in self.traces_df.columns:
            ...
            if max_tokens > avg_tokens * 5 and avg_tokens > 0:
                high_token_traces = self.traces_df[
                    self.traces_df["total_tokens"] > avg_tokens * 3
                ]
                alerts.append(
                    {
                        "type": "token_spike",
        ...
        health_checks = {
            ...
            "performance": "ok"
            if not any(a["type"] == "performance_degradation" for a in alerts)
            else "issue",
            "usage": "ok"
            if not any(a["type"] == "usage_drop" for a in alerts)
            else "issue",
            "categorization": "ok"
            if not any(a["type"] == "categorization_issue" for a in alerts)
            else "issue",
            "retention": "ok"
            if not any(a["type"] == "user_retention" for a in alerts)
            else "issue",
        }
    ...
    def analyze_query_intelligence(self) -> Dict:
        ...
        for idx, row in queries_df.iterrows():
            ...
            # City names
            cities = re.findall(
                r"\b(austin|round\s*rock|georgetown|pflugerville|cedar\s*park|leander|buda|kyle|san\s*marcos|bastrop|taylor|hutto|manor)\b",
                query,
                re.IGNORECASE,
            )
            ...
            if row.get("trace_type") == "action_plans":
                ...
                w in query for w in ["how", "what is", "explain", "tell me about"]
            ):
                query_analysis["intent"] = "information_seeking"
            elif any(
                w in query for w in ["find", "search", "looking for", "need", "where"]
            ):
            ...
            complexity = 0
            complexity += min(
                len(query_analysis["demographics"]) * 2, 4
            )  # Up to 4 points for demographics
            complexity += min(
                len(query_analysis["services"]) * 1.5, 3
            )  # Up to 3 points for services
            complexity += min(
                len(query_analysis["locations"]), 2
            )  # Up to 2 points for locations
            complexity += (
                1 if query_analysis["urgency"] in ["critical", "high"] else 0
            )  # 1 point for urgency
            ...
            if not query_analysis["locations"]:
                query_analysis["suggestions"].append(
                    "Add location (city or zip code) for more relevant results"
                )
            if not query_analysis["demographics"] and not query_analysis["services"]:
                query_analysis["suggestions"].append(
                    "Specify client situation or service type needed"
                )
            if len(row["query"]) < 30:
                query_analysis["suggestions"].append(
                    "Provide more details about the client's needs"
                )
            if query_analysis["complexity"] > 7:
                query_analysis["suggestions"].append(
                    "Consider breaking into multiple focused queries"
                )
        ...
        successful_patterns = []
        for aq in analyzed_queries:
            if aq["trace_type"] == "action_plans" or (
                aq["quality_score"] >= 70 and aq["services"]
            ):
        ...
        problematic_queries = [
            aq for aq in analyzed_queries if aq["quality_score"] < 50
        ]
        ...
        return {
            ...
            "entity_summary": {
                "top_demographics": dict(
                    sorted(
                        entity_counts["demographics"].items(),
                        key=lambda x: x[1],
                        reverse=True,
                    )[:10]
                ),
            ...
            "quality_stats": {
                ...
                "needs_improvement_pct": sum(1 for q in quality_scores if q < 50)
                / len(quality_scores)
                * 100
                if quality_scores
                else 0,
            ...
            "common_patterns": dict(
                sorted(query_templates.items(), key=lambda x: len(x[1]), reverse=True)[
                    :10
                ]
            ),
            "sample_high_quality": [
                aq for aq in analyzed_queries if aq["quality_score"] >= 70
            ][:5],
            "sample_needs_improvement": problematic_queries[:5],
            "all_analyzed": analyzed_queries[:50],  # First 50 for detailed view
            "insights": self._generate_query_insights(
                analyzed_queries, entity_counts, urgency_distribution
            ),
        }

    def _generate_query_insights(
        self, analyzed_queries: List, entity_counts: Dict, urgency_dist: Dict
    ) -> List[str]:
        ...
        if demos:
            ...
            insights.append(
                f"Most common client demographic: {top_demo[0].replace('_', ' ').title()} ({top_demo[1]} queries)"
            )

        # Service demand insights
        services = entity_counts.get("services", {})
        if services:
            top_services = sorted(services.items(), key=lambda x: x[1], reverse=True)[
                :3
            ]
        ...
        # Urgency insights
        critical_pct = (
            urgency_dist.get("critical", 0) / len(analyzed_queries) * 100
            if analyzed_queries
            else 0
        )
        if critical_pct > 10:
            insights.append(
                f"⚠️ {critical_pct:.0f}% of queries indicate critical urgency - consider priority handling"
            )
        ...
        location_rate = (
            queries_with_location / len(analyzed_queries) * 100
            if analyzed_queries
            else 0
        )
        if location_rate < 50:
            insights.append(
                f"Only {location_rate:.0f}% of queries include location - encourage users to specify area"
            )
        ...
        if low_quality > len(analyzed_queries) * 0.3:
            insights.append(
                f"📝 {low_quality} queries ({low_quality / len(analyzed_queries) * 100:.0f}%) need improvement - consider query templates"
            )
        ...
        if high_complexity > 5:
            insights.append(
                f"🔀 {high_complexity} complex multi-need queries detected - may benefit from case management approach"
            )
    ...
    def analyze_output_quality(self, return_full: bool = False) -> Dict:
        ...
        issues = {
            ...
            "location_mismatch": [
                a for a in output_analyses if not a["location_aligned"]
            ],
            "category_mismatch": [
                a for a in output_analyses if not a["category_aligned"]
            ],
            "low_quality": [a for a in output_analyses if a["quality_score"] < 50],
            "generic_response": [
                a for a in output_analyses if a.get("is_generic", False)
            ],
        ...
        return {
            ...
            "alignment_stats": {
                "location_match_rate": sum(
                    1 for a in output_analyses if a["location_aligned"]
                )
                / len(output_analyses)
                * 100
                if output_analyses
                else 0,
                "category_match_rate": sum(
                    1 for a in output_analyses if a["category_aligned"]
                )
                / len(output_analyses)
                * 100
                if output_analyses
                else 0,
                "has_resources_rate": sum(
                    1 for a in output_analyses if a["resource_count"] > 0
                )
                / len(output_analyses)
                * 100
                if output_analyses
                ...
                "actionable_rate": sum(
                    1 for a in output_analyses if a["has_actionable_info"]
                )
                / len(output_analyses)
                * 100
                if output_analyses
            ...
            "best_responses": sorted(
                output_analyses, key=lambda x: x["quality_score"], reverse=True
            )[:10],
        }
            ...
            for _, span in trace_spans.iterrows():
                ...
                if isinstance(output_attr, str) and output_attr:
                    ...
                    resources_found.extend(
                        self._extract_resources_from_text_content(output_attr)
                    )
                ...
                if isinstance(attrs, dict):
                    ...
                    if isinstance(output_data, dict):
                        ...
                        if isinstance(output_value, str) and output_value:
                            ...
                            resources_found.extend(
                                self._extract_resources_from_text_content(output_value)
                            )
                    ...
                    if isinstance(llm_data, dict):
                        ...
                        for reply in replies:
                            if isinstance(reply, str):
                                output_text += reply
                                resources_found.extend(
                                    self._extract_resources_from_text_content(reply)
                                )
                    ...
                    for key in ["output.value", "llm.output_messages"]:
                        if key in attrs:
                            ...
                            if isinstance(val, str):
                                output_text += val
                                resources_found.extend(
                                    self._extract_resources_from_text_content(val)
                                )
                                ...
                                for v in val:
                                    if isinstance(v, dict):
                                        ...
                                        output_text += content
                                        resources_found.extend(
                                            self._extract_resources_from_text_content(
                                                content
                                            )
                                        )
                ...
                if isinstance(parsed, dict):
                    ...
                    for item in parsed:
                        if isinstance(item, dict) and (
                            "name" in item or "title" in item or "organization" in item
                        ):
                            resources.append(item)
            except json.JSONDecodeError:
                pass
    ...
    def _calculate_resource_detail_stats(self, resources: List[Dict]) -> Dict:
        ...
        for res in resources:
            ...
            has_contact = any(
                bool(res.get(k)) for k in ["phone", "website", "url", "email"]
            )
    ...
    def _score_output_quality(
        ...
        has_address = bool(
            re.search(
                r"\b\d+\s+\w+\s+(st|street|ave|avenue|blvd|rd|road|dr|drive|ln|lane)\b",
                output_lower,
            )
        )
        has_website = bool(
            re.search(r"https?://|www\.|\.(org|com|gov|net)\b", output_lower)
        )
        ...
        if query_location or query_zip:
            ...
            location_found = any(
                term in output_lower for term in location_terms if len(term) > 2
            )
            location_aligned = (
                location_found or not query_location
            )  # Aligned if found OR no location requested
        ...
        if trace_type == "action_plans":
            ...
            has_timeline = bool(
                re.search(
                    r"(week|day|month|timeline|deadline|by\s+\w+day)", output_lower
                )
            )
    ...
    def _generate_output_insights(self, analyses: List, issues: Dict) -> List[str]:
        ...
        if low_quality_pct > 20:
            insights.append(
                f"⚠️ {low_quality_pct:.0f}% of responses are low quality - review AI prompts/configuration"
            )
        ...
        if loc_mismatch_pct > 15:
            insights.append(
                f"📍 {loc_mismatch_pct:.0f}% of responses don't match requested location - check geographic filtering"
            )
        ...
        if cat_mismatch_pct > 20:
            insights.append(
                f"🏷️ {cat_mismatch_pct:.0f}% of responses don't match the category requested - review category routing"
            )
        ...
        if no_resources_pct > 30:
            insights.append(
                f"📋 {no_resources_pct:.0f}% of referral responses contain no resources - expand resource database"
            )
        ...
        if generic_pct > 10:
            insights.append(
                f"🔄 {generic_pct:.0f}% are generic/no-result responses - improve fallback handling"
            )

        # Actionability
        actionable_pct = (
            sum(1 for a in analyses if a["has_actionable_info"]) / total * 100
        )
        if actionable_pct < 60:
            insights.append(
                f"📞 Only {actionable_pct:.0f}% include contact info (phone/address/website) - enhance resource data"
            )
        else:
            insights.append(
                f"✅ {actionable_pct:.0f}% of responses include actionable contact information"
            )
    ...
    def analyze_resource_recommendations(self) -> Dict:
        ...
        for _, span in self.df.iterrows():
                ...
                try:
                    ...
                    if isinstance(output_data, dict):
                        for key in [
                            ...
                            if key in output_data and isinstance(
            ...
            for res in resources_to_check:
                ...
                resource_info = {
                    "trace_id": trace_id,
                    "name": res.get(
                        "name", res.get("title", res.get("organization", "Unknown"))
                    ),
                    "category": res.get("category", res.get("type", "Unknown")),
                    "address": res.get("address", res.get("location", "")),
                    "city": res.get("city", ""),
                    "zip_code": res.get(
                        "zip", res.get("zip_code", res.get("postal_code", ""))
                    ),
                    "phone": res.get("phone", res.get("telephone", "")),
                    "website": res.get("website", res.get("url", "")),
                    "description": res.get("description", res.get("summary", ""))[:200]
                    if res.get("description") or res.get("summary")
                    else "",
                }
        ...
        concentration_ratio = (
            top_5_count / total_recommendations * 100
        ...
        central_texas_zips = [
            z
            for z in zip_distribution.keys()
            if z.startswith("78") or z.startswith("76")
        ]
        out_of_region_zips = [
            z for z in zip_distribution.keys() if z not in central_texas_zips
        ]
        ...
        for trace_id, resources in resource_by_trace.items():
            ...
            query_location = str(
                trace_row.iloc[0].get("location_preference", "")
            ).lower()
            ...
            if query_zip or query_location:
                ...
                for res in resources:
                    ...
                    if query_zip and res_zip and query_zip[:3] == res_zip[:3]:
                        ...
                        break
                    elif query_location and (
                        query_location in res_city or query_location in res_address
                    ):
                        location_match_count += 1
                        break

        location_match_rate = (
            location_match_count / total_with_location * 100
        ...
        return {
            ...
            "recommendations_per_trace": total_recommendations / len(resource_by_trace)
            if resource_by_trace
            else 0,
            "top_resources": [
                {
                    "name": name,
            ...
            "concentration_ratio": concentration_ratio,
            "diversity_score": min(100, unique_resources / total_recommendations * 100)
            if total_recommendations > 0
            else 0,
            ...
            "geographic_distribution": {
                "by_zip": dict(
                    sorted(zip_distribution.items(), key=lambda x: x[1], reverse=True)[
                        :15
                    ]
                ),
                "by_city": dict(
                    sorted(city_distribution.items(), key=lambda x: x[1], reverse=True)[
                        :10
                    ]
                ),
                "central_texas_count": sum(
                    zip_distribution[z] for z in central_texas_zips
                ),
                "out_of_region_count": sum(
                    zip_distribution[z] for z in out_of_region_zips
                ),
    ...
    def _extract_resources_from_text(self) -> Dict:
        ...
        if not resource_mentions:
            return {
                ...
                "top_resources": [],
                "insights": [
                    "No structured resource data found - consider enhancing output logging"
                ],
        ...
        return {
            ...
            "top_resources": [
                ...
                for name, count in sorted(
                    resource_mentions.items(), key=lambda x: x[1], reverse=True
                )[:20]
            ],
            "insights": [
                f"Found {len(resource_mentions)} unique resource mentions in output text"
            ],
    ...
    def _generate_resource_insights(
        ...
        if concentration > 50:
            insights.append(
                f"⚠️ Top 5 resources account for {concentration:.0f}% of all recommendations - consider diversifying"
            )
        elif concentration < 20:
            insights.append(
                f"✅ Good resource diversity - top 5 resources account for only {concentration:.0f}% of recommendations"
            )
        ...
        if top_resources:
            ...
            insights.append(
                f"📊 Most recommended resource: {top_name} ({top_count} times, {top_count / total * 100:.0f}%)"
            )

        # Location matching
        if match_rate < 70:
            insights.append(
                f"📍 Only {match_rate:.0f}% of recommendations match query location - improve geographic targeting"
            )
        else:
            insights.append(
                f"✅ {match_rate:.0f}% location match rate between queries and recommended resources"
            )
        ...
        if unique < 10:
            insights.append(
                f"📋 Only {unique} unique resources being recommended - expand resource database"
            )
    ...
    def _run_failure_taxonomy(
        ...
        for analysis in output_analyses:
            ...
            if analysis.get("resource_count", 0) > 0 and not analysis.get(
            ...
            if (
                analysis.get("trace_type") == "referrals"
                and analysis.get("resource_count", 0) == 0
            ):
                ...
                has_resource_content = any(
                    ind in output_text_check for ind in resource_indicators
                )
        ...
        return {
            ...
            "failure_rate": traces_with_failures / len(output_analyses) * 100
            if output_analyses
            else 0,
            "failures_by_type": {
                k: v[:20] for k, v in failures.items()
            },  # Top 20 examples each
    ...
    def _run_resource_validation(
                    ...
                    matched_list.append(
                        {
                            ...
                            "matched_to": inventory_index[normalized].get(
                                "name", res_name
                            ),
                            "trace_id": analysis.get("trace_id"),
                        }
        ...
        match_rate = (
            matched_resources / total_resources * 100 if total_resources > 0 else 0
        )
        ...
        return {
            ...
            "health": "good"
            if match_rate >= 90
            else ("warning" if match_rate >= 70 else "critical"),
        }
    ...
    def _run_coverage_analysis(
        ...
        overall_coverage = (
            total_recommended / total_in_inventory * 100
            if total_in_inventory > 0
            else 0
        )
        ...
        for cat, resources in inventory_by_category.items():
            ...
            category_coverage[cat] = {
                ...
                "overlap": overlap,
                "coverage_pct": overlap / len(cat_in_inventory) * 100
                if cat_in_inventory
                else 0,
            }
        ...
        return {
            ...
            "category_coverage": dict(
                sorted(category_coverage.items(), key=lambda x: x[1]["coverage_pct"])
            ),
    ...
    def _run_test_case_evaluation(
        ...
        for tc in test_cases:
            ...
            matching_traces = [
                a
                for a in self._build_output_analyses()
            ...
            # Calculate metrics
            resource_recall = (
                len(actual_resources & expected_resources)
            ...
            resource_precision = (
                len(actual_resources & expected_resources) / len(actual_resources) * 100
                if actual_resources
                else 0
            )

            category_match = (
                expected_category in (trace.get("query_category") or "").lower()
                if expected_category
                else True
            )
            location_match = (
                expected_location in (trace.get("output_text") or "").lower()
    ...
    def _build_executive_summary(
        ...
        # Actionability
        actionable_rate = (
            sum(1 for a in output_analyses if a.get("has_actionable_info", False))
            / total
            * 100
            if total
            else 0
    ...
    def _generate_exec_recommendations(
        ...
        for failure_type, count in top_failures:
            if failure_type == "hallucination" and count > 0:
                recs.append(
                    f"⚠️ {count} potential hallucinations detected - verify resource inventory"
                )
            elif failure_type == "location" and count > 5:
                recs.append(
                    f"📍 {count} location mismatches - improve geographic targeting"
                )
                ...
                recs.append(
                    f"📋 {count} queries returned no resources - expand database"
                )
            elif failure_type == "generic" and count > 5:
                recs.append(
                    f"📝 {count} generic responses - improve prompt specificity"
                )

        if match_rate < 80:
            recs.append(
                f"🔍 Only {match_rate:.0f}% of resources match inventory - investigate data gaps"
            )

        if actionable_rate < 70:
            recs.append(
                f"📞 Only {actionable_rate:.0f}% include contact info - enhance resource data"
            )
    ...
    def run_consistency_analysis(
        ...
        inconsistent_count = sum(
            1 for r in consistency_results if not r["is_consistent"]
        )

        return {
            "groups_analyzed": len(consistency_results),
            "inconsistent_groups": inconsistent_count,
            "consistency_rate": (len(consistency_results) - inconsistent_count)
            / len(consistency_results)
            * 100
            if consistency_results
            else 100,
            "most_inconsistent": consistency_results[:10],
            "most_consistent": consistency_results[-10:]
