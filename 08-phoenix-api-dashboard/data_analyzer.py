...
class TraceAnalyzer:
    ...
    def get_macro_statistics(self) -> Dict:
        ...
        stats = {
            ...
            'unique_traces': self.df['trace_id'].nunique() if 'trace_id' in self.df.columns else 0,
            'trace_counts': trace_counts,
            'date_range': {
                'start': self.df['start_time'].min() if 'start_time' in self.df.columns else None,
                'end': self.df['start_time'].max() if 'start_time' in self.df.columns else None
            },
            'avg_latency_s': self.df['latency_s'].mean() if 'latency_s' in self.df.columns else 0,
            'median_latency_s': self.df['latency_s'].median() if 'latency_s' in self.df.columns else 0,
            'p95_latency_s': self.df['latency_s'].quantile(0.95) if 'latency_s' in self.df.columns else 0,
            'p99_latency_s': self.df['latency_s'].quantile(0.99) if 'latency_s' in self.df.columns else 0,
            'total_tokens': self.df['token_count_total'].sum() if 'token_count_total' in self.df.columns else 0,
            'avg_tokens_per_request': self.df['token_count_total'].mean() if 'token_count_total' in self.df.columns else 0,
            'error_rate': (self.df['status_code'] == 'ERROR').sum() / len(self.df) * 100 if 'status_code' in self.df.columns else 0,
            'success_rate': (self.df['status_code'] == 'OK').sum() / len(self.df) * 100 if 'status_code' in self.df.columns else 0,
        }
